...
namespace AkiGames.UI
{
    public class UITransform : GameComponent, IAlignable
    {
        ...
        [DontSerialize] public Rectangle LocalBounds
        {
            ...
            set
            {
                OffsetMin = new Vector2(value.X, value.Y);
                OffsetMax = new Vector2(value.X + value.Width, value.Y + value.Height);
                Width = value.Width;
                Height = value.Height;
            }
        }
        ...
        public void ForceSetBounds(Rectangle bounds)
        {
            Bounds = bounds;
            Width = bounds.Width;
        ...
        [HideInInspector] public Vector2 OriginPosition => new Vector2(
        ...
        {
            if (parentTransform == null && gameObject.Parent != null)
        ...
        // === Реализация IAlignable ===
        (int, int) IAlignable.PositionAfterTurningParent(UITransform parentTransform, int x, int y)
        {
            if (parentTransform.Rotation == 0) return (x, y);
            Vector2 pivot = new(parentTransform.OriginPosition.X, parentTransform.OriginPosition.Y);
            Vector2 point = new(x, y);
            Vector2 rotated = RotatePointAround(point, pivot, -parentTransform.Rotation * (float)(Math.PI / 180.0));
            return ((int)rotated.X, (int)rotated.Y);
        }

        private static Vector2 RotatePointAround(Vector2 point, Vector2 pivot, float angleRad)
        {
            float dx = point.X - pivot.X;
            float dy = point.Y - pivot.Y;
            float cos = (float)Math.Cos(angleRad);
            float sin = (float)Math.Sin(angleRad);
            float x = dx * cos - dy * sin + pivot.X;
            float y = dx * sin + dy * cos + pivot.Y;
            return new Vector2(x, y);
        }

        // === Contains с учётом поворота ===
        public bool Contains(Point point)
        {
            if (Rotation == 0) return Bounds.Contains(point);
            Vector2 pointRotated = LocalPosition(new Vector2(point.X, point.Y));
            return Bounds.Contains(new Point((int)pointRotated.X, (int)pointRotated.Y));
        }

        private Vector2 LocalPosition(Vector2 globalPoint)
        {
            Vector2 pivot = new(OriginPosition.X, OriginPosition.Y);
            return RotatePointAround(globalPoint, pivot, (float)(-Rotation * Math.PI / 180.0));
        }

        // === Статический фабричный метод ===
        public static UITransform TransformOfBounds(Rectangle bounds) => new UITransform { Bounds = bounds };
    }
}